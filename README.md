...
A token is used to validate the client when making API requests.

- [x] What steps can you take in your web apps to keep your data secure?

Add protected routes so that only logged-in users can view certain pages.

- [x] Describe how web servers work.

Web servers store information on different websites. Specific websites are linked to a DNS and when a user (client) tries to load a specific DNS/Domain in the browser, it'll find and fetch the necessary information from a server.
